...
import type { BoilerReplacementInquiryData } from "@/lib/supabase/service-requests"
import {
  submitBoilerReplacementServiceRequest,
  uploadBoilerReplacementImages,
...
const boilerStatusOptions = ["Igen", "Nem", "Részben"] as const
...
const initialFormState: BoilerReplacementInquiryData = {
  property: {
    propertyType: "",
    area: "",
    postalCode: "",
...
const fieldLabels: Record<FieldName, string> = {
  ...
  city: "Település",
  currentBoilerType: "Jelenlegi kazán típusa",
  boilerWorkingStatus: "Működik még?",
  replacementReason: "Miért aktuális a megkeresés?",
  name: "Név",
  phone: "Telefonszám",
  email: "E-mail",
...
const requiredFieldsByStep = [
  ["propertyType", "area", "postalCode", "city"],
  ["currentBoilerType", "boilerWorkingStatus", "replacementReason"],
  [
    "name",
    "phone",
    "email",
...
const summarySections = [
  {
    title: "Ingatlan adatai",
    fields: ["propertyType", "area", "postalCode", "city"],
  },
  {
    title: "Jelenlegi rendszer",
    fields: ["currentBoilerType", "boilerWorkingStatus", "replacementReason"],
  },
  {
    title: "Kapcsolatfelvétel",
...
function getFieldValue(
  ...
  switch (field) {
    ...
    case "currentBoilerType":
      return formData.currentSystem.currentBoilerType
    case "boilerWorkingStatus":
      return formData.currentSystem.boilerWorkingStatus
    case "replacementReason":
      return formData.currentSystem.replacementReason
    case "name":
...
function validateField(
  ...
  switch (field) {
    ...
    case "currentBoilerType":
      return value ? "" : "Adja meg a jelenlegi kazán típusát."
    case "boilerWorkingStatus":
      return value ? "" : "Jelölje, hogy a kazán működik-e még."
    case "replacementReason":
      return value ? "" : "Válassza ki, miért aktuális a megkeresés."
    case "name":
...
export function BoilerReplacementForm() {
  ...
  async function submitForm() {
    ...
    if (Object.keys(nextErrors).length > 0) {
      if (nextErrors.propertyType || nextErrors.area || nextErrors.city) {
        setCurrentStep(0)
      } else if (
        nextErrors.currentBoilerType ||
        nextErrors.boilerWorkingStatus ||
        nextErrors.replacementReason
      ) {
        setCurrentStep(1)
      } else {
        setCurrentStep(2)
    ...
    try {
      ...
      await submitBoilerReplacementServiceRequest({
        requestId,
        formData,
      })
      ...
      if (selectedImages.length > 0) {
        try {
          const attachments = await uploadBoilerReplacementImages({
            requestId,
          ...
          logClientError("Kazáncsere képfeltöltés hiba", error, {
            serviceType: "kazancsere",
          })
      ...
      logClientError("Kazáncsere ajánlatkérés mentési hiba", error, {
        serviceType: "kazancsere",
      })
      ...
      <Card className={cn(formMainCardClassName, "order-1 lg:order-2")}>
        ...
        <CardContent className="space-y-5 pt-4 sm:space-y-7 sm:pt-5 md:space-y-8 md:pt-6">
          ...
          {currentStep === 1 ? (
            <div className="space-y-6">
              <label className="space-y-3">
                  <FormFieldLabel label="Jelenlegi kazán típusa" required />
                <Input
                  type="text"
                  placeholder="Például: kondenzációs gázkazán"
                  value={formData.currentSystem.currentBoilerType}
                  onChange={(event) =>
                    updateField("currentBoilerType", event.target.value)
                  }
                  onBlur={() => handleFieldBlur("currentBoilerType")}
                  aria-invalid={getFieldState("currentBoilerType").invalid}
                  disabled={isSubmitting}
                  className={cn(
                    "h-12",
                    getFormInputClassName(getFieldState("currentBoilerType"))
                  )}
                />
                {getFieldError("currentBoilerType") ? (
                  <FormFieldMessage
                    message={getFieldError("currentBoilerType") ?? ""}
                  />
                ) : (
                  <FormFieldMessage
                    tone="muted"
              ...
              <FormOptionGroup
                label="Működik még?"
                options={boilerStatusOptions}
                value={formData.currentSystem.boilerWorkingStatus}
                error={getFieldError("boilerWorkingStatus")}
                required
                disabled={isSubmitting}
                onChange={(value) =>
                  updateField("boilerWorkingStatus", value, { touch: true })
                }
              />

              <FormOptionGroup
                label="Miért aktuális a megkeresés?"
                options={replacementReasonOptions}
                value={formData.currentSystem.replacementReason}
                error={getFieldError("replacementReason")}
                required
                disabled={isSubmitting}
                onChange={(value) =>
                  updateField("replacementReason", value, { touch: true })
                }
              />
            </div>
          ) : null}
          ...
          {currentStep === 2 ? (
            <div className="space-y-6">
              ...
              <label className="space-y-3">
                ...
                <Textarea
                  placeholder="Röviden leírhatja a helyszínt, az igényt vagy bármilyen fontos körülményt."
