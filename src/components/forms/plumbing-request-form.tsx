...
import type { PlumbingRequestInquiryData } from "@/lib/supabase/service-requests"
import {
  submitPlumbingServiceRequest,
...
const workTypeOptions = [
  ...
  "Szaniter csere",
  "Lefolyó probléma",
  "Gázvezeték vagy gázkészülék szerelés",
  "Gázkészülék csere",
  "Általános víz- vagy gázszerelés",
  "Egyéb",
] as const
...
const initialFormState: PlumbingRequestInquiryData = {
  plumbing: {
    workType: "",
    urgency: "",
    description: "",
...
const fieldLabels: Record<FieldName, string> = {
  workType: "Milyen jellegű munkáról van szó?",
...
const requiredFieldsByStep = [
  ["workType", "urgency", "description"],
  ["postalCode", "city", "preferredContactTime", "siteVisitNeed", "appointmentWindow"],
  ["name", "phone", "email"],
  [],
...
const summarySections = [
  {
    title: "Munka jellege",
    fields: ["workType", "urgency", "description"],
  },
  {
    title: "Helyszín és egyeztetés",
    fields: [
...
function getFieldValue(formData: PlumbingRequestInquiryData, field: FieldName) {
  switch (field) {
    case "workType":
      return formData.plumbing.workType
    case "urgency":
      return formData.plumbing.urgency
    case "description":
      return formData.plumbing.description
    case "postalCode":
...
function validateField(field: FieldName, formData: PlumbingRequestInquiryData) {
  const value = getFieldValue(formData, field).trim()

  switch (field) {
    case "workType":
      return value ? "" : "Válassza ki a munka jellegét."
    case "urgency":
...
export function PlumbingRequestForm() {
  ...
  async function submitForm() {
    ...
    if (Object.keys(nextErrors).length > 0) {
      if (nextErrors.workType || nextErrors.urgency || nextErrors.description) {
        setCurrentStep(0)
      } else if (
        nextErrors.city ||
    ...
    try {
      ...
      await submitPlumbingServiceRequest({
        requestId,
        formData,
      })
      ...
      if (selectedImages.length > 0) {
        try {
          ...
          logClientError("Vízszerelési képfeltöltés hiba", error, {
            serviceType: "vizszereles",
          })
      ...
      logClientError("Vízszerelési megkeresés mentési hiba", error, {
        serviceType: "vizszereles",
      })
      ...
      setSubmitErrorMessage(
        getUserFacingErrorMessage(
          error,
          "A szerelési megkeresés rögzítése most nem sikerült. Kérjük, próbálja meg újra."
        )
      )
    }
  }
        ...
        <CardContent className="space-y-5 pt-4 sm:space-y-7 sm:pt-5 md:space-y-8 md:pt-6">
          ...
          {currentStep === 0 ? (
            <div className="space-y-6">
              <FormOptionGroup
                label="Milyen jellegű munkáról van szó?"
                options={workTypeOptions}
                value={formData.plumbing.workType}
                error={getFieldError("workType")}
                required
                disabled={isSubmitting}
                onChange={(value) => updateField("workType", value, { touch: true })}
              />

              <FormOptionGroup
                label="Mennyire sürgős?"
                options={urgencyOptions}
                value={formData.plumbing.urgency}
              ...
              <label className="space-y-3">
                ...
                <Textarea
                  placeholder="Röviden írja le a feladatot, a problémát vagy azt, hogy milyen jellegű beavatkozásra lenne szükség."
                  value={formData.plumbing.description}
          ...
          {currentStep === 2 ? (
            <div className="space-y-6">
              ...
              <label className="space-y-3">
                ...
                <Textarea
                  placeholder="Ha van plusz helyszíni vagy szervezési megjegyzése, itt röviden jelezheti."
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-base font-medium text-white">
                        {hasSelectedImages
                          ? "A szerelési megkeresés rögzítése és a képek feltöltése folyamatban van."
                          : "A szerelési megkeresés rögzítése folyamatban van."}
                      </p>
              ...
              {isError ? (
                <div className={formErrorPanelClassName}>
                  <p className="text-base font-medium text-rose-50">
                    A szerelési megkeresés rögzítése most nem sikerült.
                  </p>
