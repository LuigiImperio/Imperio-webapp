...
Az admin dashboard helyi megnyitása első belépéskor:

- `/admin?token=SAJAT_TOKEN`

Az admin ajánlatkérés-lista helyi megnyitása első belépéskor:

- `/admin/ajanlatkeresek?token=SAJAT_TOKEN`

Ehhez az `ADMIN_ACCESS_TOKEN` változót is be kell állítani.

Sikeres tokenellenőrzés után a rendszer HTTP-only sütibe menti a belső hozzáférést, majd a tokent eltávolítja az URL-ből. A további admin navigáció már nem viszi tovább a tokent query paraméterként.
...
Az admin dashboard ugyanazzal a belső hozzáférési védelemmel működik, mint a részletes ajánlatkéréslista.
